...
function UserVinRequestChat() {
    ...
    const getSenderName = (msg) => {
        if (msg.senderId === user?.id) {
            return null; // Я, не показываем имя
        }
    ...
    return (
        <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
            <button onClick={() => navigate('/profile')} style={{ marginBottom: '20px', cursor: 'pointer' }}>
                ← Назад в профиль
            </button>

            <h1>VIN-заявка #{id}</h1>
            {vinRequest && (
                <div style={{ backgroundColor: '#f5f5f5', padding: '10px', borderRadius: '8px', marginBottom: '20px' }}>
                    <p><strong>VIN:</strong> {vinRequest.vin}</p>
                    <p><strong>Описание:</strong> {vinRequest.description || '-'}</p>
                    <p><strong>Статус:</strong> {vinRequest.status}</p>
                </div>
            )}
